...
public class CrystalUtils {
    public static boolean CanPlaceCrystalIfObbyWasAtPos(final BlockPos pos)
    {
        ...
        final Block floor = Minecraft.getMinecraft().world.getBlockState(pos.add(0, 1, 0)).getBlock();
        final Block ceil = Minecraft.getMinecraft().world.getBlockState(pos.add(0, 2, 0)).getBlock();

        if (floor == Blocks.AIR && ceil == Blocks.AIR)
        {
            if (Minecraft.getMinecraft().world.getEntitiesWithinAABBExcludingEntity(null, new AxisAlignedBB(pos.add(0, 1, 0))).isEmpty())
            {
                return true;
            }
        }
            ...
            int p_InterlopedAmount)
    {
        ...
        {
            Vec3d l_Interloped = PositionUtils.getInterpolatedAmount(entity, p_InterlopedAmount);
            l_Distance = PositionUtils.GetDistance(l_Interloped.x, l_Interloped.y, l_Interloped.z, posX, posY, posZ);
        }
